...
const PREDEFINED_MISSIONS: Record<string, Mission> = {
    'coffee-shop': {
        _id: 'coffee-shop',
...
export default function MissionChatPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        params.then(({ id }) => {
            setMissionId(id);
            const foundMission = PREDEFINED_MISSIONS[id];
            if (foundMission) {
                setMission(foundMission);
                // Send initial greeting
                setMessages([{
                    role: 'assistant',
                    content: 'Hello! Welcome to the scenario. Let\'s begin!',
                }]);
            }
        });
    }, [params]);
