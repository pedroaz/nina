"""Unit tests for the opencode password file management."""
...
import pytest
from nina_core.opencode.password import (
...
def test_ensure_password_file_creates_with_0600(tmp_path: Path) -> None:
    path = ensure_password_file(tmp_path, "opencode_password")
    assert path.exists()
...
def test_ensure_password_file_is_idempotent(tmp_path: Path) -> None:
    ...
    path2 = ensure_password_file(tmp_path, "opencode_password")
...
def test_ensure_password_file_force_regenerates(tmp_path: Path) -> None:
    path = ensure_password_file(tmp_path, "opencode_password")
    first = path.read_text()
    path2 = ensure_password_file(tmp_path, "opencode_password", force=True)
    second = path2.read_text()
    assert first != second
...
def test_read_password_round_trips(tmp_path: Path) -> None:
    ...
    assert read_password(tmp_path, "opencode_password") == expected
